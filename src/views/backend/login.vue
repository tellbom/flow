...
const onSubmit = (captchaInfo = "") => {
  ...
  login("post", {
    ...
    .then((res) => {
      console.log(res.data.userInfo, adminBaseRoutePath);
...
import { onMounted, onBeforeUnmount, reactive, ref, nextTick } from "vue";
import * as pageBubble from "/@/utils/pageBubble";
import type { FormInstance, InputInstance,ElMessage } from "element-plus";
import { editDefaultLang } from "/@/lang/index";
import { useConfig } from "/@/stores/config";
import { useAdminInfo } from "/@/stores/adminInfo";
import { rbacLogin } from "/@/api/backend/rbac/index.ts";
import { initRbacBridge } from "/@/api/backend/rbac/bridge.ts";
import { uuid } from "/@/utils/random";
import { buildValidatorData } from "/@/utils/validate";
import router from "/@/router";
import toggleDark from "/@/utils/useDark";
...
onMounted(() => {
  rbacLogin()
    .then(async (res:any) => {
      adminInfo.dataFill(res.adminInfo)

      // ── 新增：RBAC Bridge ──────────────────────────────────────
      const bridge = await initRbacBridge()
      if (!bridge.success) {
        // token 被 RBAC 拒绝（未授权该 project）
        ElMessage.error('无权限进入系统，请联系管理员')
        adminInfo.removeToken()
        return  // 停在登录页，不跳转
      }
      router.push({ path: adminBaseRoutePath })   // 移到这里
      // ── 新增结束 ──────────────────────────────────────────────
